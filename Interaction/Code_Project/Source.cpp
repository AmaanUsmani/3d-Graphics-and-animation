...
#include "include/glad/glad.h" // Extend OpenGL Specfication to version 4.5 for WIN64 and versions 4.1 for Apple (note: two different files).

#include "include/imgui/imgui.h"			  // Load GUI library - Dear ImGui - https://github.com/ocornut/imgui
#include "include/imgui/imgui_impl_glfw.h"	  // Platform ImGui using GLFW
#include "include/imgui/imgui_impl_opengl3.h" // Platform new OpenGL - aka better than 3.3 core version.

#include "include/GLFW/glfw3.h" // Add GLFW - library to launch a window and callback mouse and keyboard - https://www.glfw.org

#define GLM_ENABLE_EXPERIMENTAL			 // Enable all functions include non LTS
#include "include/glm/glm.hpp"			 // Add helper maths library - GLM 0.9.9.9 - https://github.com/g-truc/glm - for example variables vec3, mat and operators.
#include "include/glm/gtx/transform.hpp" // Help us with transforms
using namespace glm;
...
int main()
{
	cout << endl
		 << "===" << endl
		 << "3D Graphics and Animation - Running..." << endl;
	...
	cout << endl
		 << "Starting main loop and rendering..." << endl;
...
void hintsGLFW()
{
	...
	auto majorVersion = 3;
	auto minorVersion = 3; // define OpenGL version - at least 3.3 for bare basic NEW OpenGL

#if defined(__WIN64__)
	majorVersion = 4;
	minorVersion = 5;									// Recommended for Windows 4.5, but latest is 4.6 (not all drivers support 4.6 version).
	glfwWindowHint(GLFW_OPENGL_DEBUG_CONTEXT, GL_TRUE); // Create context in debug mode - for debug message callback
#elif (__APPLE__)
	majorVersion = 4;
	minorVersion = 1; // Max support for OpenGL in MacOS
#endif
...
GLenum glCheckError_(const char *file, int line) // Debugger manual function for Apple - OpenGL versions 3.3 to 4.1.
{
	...
	while ((errorCode = glGetError()) != GL_NO_ERROR)
		debugger.GlGetError(errorCode, file, line);
